...
namespace caffe {
	...
	void PairFastLossLayer<Dtype>::Forward_cpu(
		const vector<Blob<Dtype>*> & bottom, const vector<Blob<Dtype>*> & top){
		//max_dist_class.clear();
		//min_dist_class.clear();
		...
		Dtype use_pos = this->layer_param_.pair_fast_loss_param().use_pos();// alpha
		...
		//record the max distance within each class data

		// for (auto const &ent : label_data_map) {
		// 	max_dist_class[ent.first] = Dtype(0);
		// 	min_dist_class[ent.first] = Dtype(10000);
		// }
		...
		for (auto const &ent1 : label_data_map)
		{
			...
			for (int j = 0; j < cur_size - 1; j++)
			{
				for (int k = j + 1; k < cur_size; k++)
				{
					...
					//max_dist_class[ent1.first] = max(max_dist_class[ent1.first], dist_matrix.cpu_data()[anc * nums + pos]);
					//min_dist_class[ent1.first] = min(min_dist_class[ent1.first], dist_matrix.cpu_data()[anc * nums + pos]);
